...
export default function PersonalityPrediction() {
    ...
    const submitAnswers = async () => {
        ...
        try {
            ...
            // Added check to prevent the JSON crash if the server returns HTML
            const contentType = response.headers.get("content-type");
            if (contentType && contentType.includes("text/html")) {
                throw new Error("Server error: The backend crashed and returned HTML instead of data.");
            }
                            ...
                            <div className="flex items-center gap-4 mb-8 border-b border-white/5 pb-6">
                                ...
                                {personalityResult && (
                                    <button
                                        ...
                                        className="px-4 py-2 text-sm bg-white/10 hover:bg-white/20 border border-white/10 rounded-lg text-white transition-all cursor-pointer shadow-sm"
                                    >
                                        Copy Result
                                    </button>
                                )}
